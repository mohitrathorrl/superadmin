  ...
  return (
    <div className="min-h-screen flex">
      <div className="hidden lg:flex w-1/2 items-center justify-center bg-zinc-50">
        <Player autoplay loop src={loginJson} className="max-w-md" style={{ height: "350px", width: "350px" }} />
      </div>
      ...
      <div className="flex w-full lg:w-1/2 min-h-screen items-center justify-center p-4">
        <div className="w-full max-w-md surface p-8 space-y-6">
          <div className="flex justify-center">
            <img src="https://fincloud-tech.s3.ap-south-1.amazonaws.com/finclouds_logo.png" alt="FinCloud Logo" className="h-6" />
          </div>

          <div>
            <h1 className="text-2xl font-bold">{step === "email" ? "Sign in to your account" : "Verify OTP"}</h1>
            <p className="mt-1 text-sm text-zinc-600">
              {step === "email" ? "Enter your email to continue" : `Enter the 6-digit code sent to ${email}`}
            </p>
          </div>
          ...
          {step === "otp" && (
            <div className="space-y-6">
              ...
              <button onClick={() => { setOtp(["", "", "", "", "", ""]); setStep("email") }} className="text-sm text-zinc-500 hover:underline w-full">
                Change email
              </button>
            </div>
          )}
